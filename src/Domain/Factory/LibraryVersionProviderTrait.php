        ...
        switch (strtolower($name)) {
            ...
            case 'enchant';
                if (defined('LIBENCHANT_VERSION')) {
                    $versionText = LIBENCHANT_VERSION;
                }
                break;
            case 'iconv':
                $versionText = ICONV_VERSION;
                break;
            case 'intl':
            case 'ICU':
                if (defined('INTL_ICU_VERSION')) {
                    $versionText = INTL_ICU_VERSION;
                }
                break;
            case 'imagick':
                ...
                break;
            case 'libmemcached':
                if (defined('Memcached::LIBMEMCACHED_VERSION_HEX')) {
                    ...
                    $versionText = Memcached::LIBMEMCACHED_VERSION_HEX;
                }
                break;
            case 'libxml':
                if (defined('LIBXML_DOTTED_VERSION')) {
                    $versionText = LIBXML_DOTTED_VERSION;
                }
                break;
            case 'openssl':
            ...
            case 'pcre':
                if (defined('PCRE_VERSION')) {
                    $versionText = preg_replace('{^(\S+).*}', '$1', PCRE_VERSION);
                }
                break;
            case 'sqlite3':
            ...
            case 'xsl':
                if (defined('LIBXSLT_DOTTED_VERSION')) {
                    $versionText = LIBXSLT_DOTTED_VERSION;
                }
                break;
            case 'zip':
                if (defined('ZipArchive::LIBZIP_VERSION')) {
                    $versionText = ZipArchive::LIBZIP_VERSION;
                }
                break;
        }

...
trait ExtensionVersionProviderTrait
{
    ...
    protected function getLatestExtVersion(string $name, string $type = 'bundle'): string
    {
        static $cache = [];

        $normalizedPackage = strtolower($name);
        ...
        if ('bundle' === $type) {
            $cache[$name] = $this->getExtensionVersion($name);
            return $cache[$name];
        }

        $rss = simplexml_load_file(sprintf('http://pecl.php.net/feeds/pkg_%s.rss', $normalizedPackage));
        if (!$rss instanceof SimpleXMLElement) {
            $cache[$name] = '';
            return $cache[$name];
        }
        $cache[$name] = trim(str_ireplace($name, '', $rss->item->title));
        return $cache[$name];
    }
